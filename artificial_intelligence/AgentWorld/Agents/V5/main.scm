...
;;this function calculate feature21, is-making-turn-to
...
;;this function modify the give-up mode to true after it is fought by other agents when eating
(define (check-give-up previous-events percepts)
  (if (and (is-vegetation? (get-location percepts 0 1))
	   (is-fought? previous-events))
      #t
      #f))
...
(define (move-passive-1-score previous-events percepts give-up)
  (let* ((origin-state (get-location percepts 0 1)))
    (+ (* (is-current-not-accessible percepts 0 1) w10)
...
(define (choose-action current-energy previous-events percepts)
  (let* ((give-up (check-give-up previous-events percepts))
	 (s-score (stay-score previous-events percepts))
	 ...
	 (mp1-score (move-passive-1-score previous-events percepts give-up))
