...
;;this function calculate feature25, is-surround-by-vegetation-2, if after moving 2 steps, it is surrounded by the vegetation energy level greater than 30, then return 1, otherwise return 0
(define (is-surround-by-vegetation-2 x y percepts)
  (let ((above (get-location percepts x (+ y 1)))
    ...
    (if (or (and (is-vegetation? above) (> (get-vegetation-energy above) 30))
	    (and (is-vegetation? left) (> (get-vegetation-energy left) 30))
	    (and (is-vegetation? right) (> (get-vegetation-energy right) 30)))
	1
	0)))

;;this function calculate feature25, is-surround-by-vegetation-3, is after moving 3 steps, it is surrounded by the vegetation energy level greater than 60, then return 1, otherwise return 0
(define (is-surround-by-vegetation-3 x y percepts)
  (let ((above (get-location percepts x (+ y 1)))
	(left (get-location percepts (- x 1) y))
	(right (get-location percepts (+ x 1) y)))
    (if (or (and (is-vegetation? above) (> (get-vegetation-energy above) 60))
	    (and (is-vegetation? left) (> (get-vegetation-energy left) 60))
	    (and (is-vegetation? right) (> (get-vegetation-energy right) 60)))
	1
	0)))
...
;;this function calculate the feature 31, turn-to-predator-penalty, if the agent turning to a predator, return 1, and it will receive penalty when calculate score, otherwise return 0
...
(define (turn-left-score previous-events percepts)
  (let* ((origin-state (get-location percepts 0 1))
    ...
    (+ (if (is-barrier? origin-state) w21 0)
       ...
       (* (turn-to-predator-penalty left-corner) w31)
...
(define (turn-right-score previous-events percepts)
  (let* ((origin-state (get-location percepts 0 1))
    ...
    (+ (if (is-barrier? origin-state) w21 0)
       ...
       (if (is-ate? previous-events) w24 0)
       (* (continuous-turn-penalty percepts) w28)
       (* (turn-to-predator-penalty right-corner) w31)
...
(define (move-passive-3-score previous-events percepts)
  (let* ((origin-state (get-location percepts 0 1))
    ...
    (+ (* (is-current-not-accessible percepts 0 3) w10)
       ...
       (* (is-surround-by-vegetation-2 0 3 percepts) w25)
       (if (has-vegetation-this-line? percepts) w15 0)
       w5)))
...
(define (choose-action current-energy previous-events percepts)
  (let* ((s-score (stay-score previous-events percepts))
    ...
    (cond ((equal? max-score s-score) "STAY")
	  ((equal? max-score tr-score) (begin (set! right-corner bottom)
					      (set! bottom left-corner)
					      (set! left-corner (get-location percepts 0 1))
					      "TURN-RIGHT"))
	  ((equal? max-score tf-score) (begin (set! left-corner bottom)
